...
public class JsonUtils1 {
	 ...
	 *            JSON字符串
	 * @param tr
	 *            例如: new TypeReference<List<FamousUser> >(){}
	 * @return
	 */
	 ...
	 * @return
	 */
	public static String toJsonWithSerializeAllExcept(Object obj, String... filterFields) {
		if (obj == null) {
			return null;
		}
		try {
			...
			FilterProvider filters = new SimpleFilterProvider().addFilter(obj.getClass().getName(),
					SimpleBeanPropertyFilter.serializeAllExcept(filterFields)).setFailOnUnknownId(false);
	 ...
	 * @return
	 */
	public static String toJsonWithFilterOutAllExcept(Object obj, String... filterFields) {
		if (obj == null) {
			return null;
		}
		try {
			...
			mapper.setSerializationConfig(mapper.getSerializationConfig().withSerializationInclusion(
					JsonSerialize.Inclusion.NON_NULL));
			FilterProvider filters = new SimpleFilterProvider().addFilter(obj.getClass().getName(),
					SimpleBeanPropertyFilter.filterOutAllExcept(filterFields)).setFailOnUnknownId(false);
			mapper.setFilters(filters);
			mapper.setAnnotationIntrospector(new JacksonAnnotationIntrospector() {
				@Override
				public Object findFilterId(AnnotatedClass ac) {
